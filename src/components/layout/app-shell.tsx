...
type Modules = { posts: boolean; rss: boolean; ai: boolean; newsletter: boolean; ads: boolean; analytics: boolean };

export async function AppShell({ active, children }: { active: string; children: React.ReactNode }) {
  const settings = await getSiteSettings();
  const modules: Modules = { posts: settings.modulePosts, rss: settings.moduleRss && isRssReaderAvailable(), ai: settings.moduleAi, newsletter: settings.moduleNewsletter, ads: settings.moduleAds, analytics: settings.moduleAnalytics };
  const routeModules: Record<string, keyof Modules> = { "/yazilar": "posts", "/rss": "rss", "/yapay-zeka": "ai", "/e-bulten": "newsletter", "/reklamlar": "ads", "/istatistik": "analytics" };
