...
type Modules = { posts: boolean; rss: boolean; ads: boolean; analytics: boolean; newsletter: boolean; localChat: boolean };
...
export async function AppShell({ active, children }: { active: string; children: React.ReactNode }) {
  ...
  const modules: Modules = { localChat: localToolsAvailable, posts: settings.modulePosts, rss: settings.moduleRss && localToolsAvailable, ads: settings.moduleAds, analytics: settings.moduleAnalytics, newsletter: settings.moduleNewsletter };
  const routeModules: Record<string, keyof Modules> = { "/yerel-asistan": "localChat", "/yazilar": "posts", "/gunun-ozeti": "posts", "/rss": "rss", "/reklamlar": "ads", "/istatistik": "analytics", "/bulten": "newsletter" };
