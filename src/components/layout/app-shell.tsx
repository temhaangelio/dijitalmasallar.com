...
export async function AppShell({ active, children }: { active: string; children: React.ReactNode }) {
  ...
  const modules: Modules = { posts: settings.modulePosts, rss: settings.moduleRss && isRssReaderAvailable(), ads: settings.moduleAds, analytics: settings.moduleAnalytics };
