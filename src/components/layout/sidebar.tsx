  ...
  return <aside className="sidebar">
    ...
    <nav aria-label="Yönetim menüsü" className="flex flex-col gap-1">
      {adminNavItems.filter(({ module }) => !module || modules[module]).map(({ label, href, icon: Icon }) => {
        ...
        return <Link key={href} href={href} aria-label={label} aria-current={selected ? "page" : undefined} title={label} className={`sidebar-item relative text-[15px] transition-colors ${selected ? "font-semibold text-ink" : "font-medium text-ink-2 hover:bg-surface-2 hover:text-ink"}`}>
          <Icon size={18} strokeWidth={selected ? 2 : 1.6} className="shrink-0" aria-hidden="true" /><span className="sidebar-expanded-only truncate">{label}</span>
        </Link>;
      })}
    </nav>
