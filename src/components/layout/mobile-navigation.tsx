...
export function MobileNavigation({ active, siteName, modules }: { active: string; siteName: string; modules: Record<"posts" | "rss" | "ads" | "analytics", boolean> }) {
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-50 bg-ink/25" role="presentation" onMouseDown={() => setOpen(false)}><aside role="dialog" aria-modal="true" aria-label="Ana menü" className="ml-auto flex h-full w-[min(88vw,360px)] flex-col bg-canvas p-5 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}><div className="mb-8 flex items-center justify-between"><strong className="text-lg">{siteName}</strong><button autoFocus aria-label="Menüyü kapat" onClick={() => setOpen(false)} className="grid size-11 place-items-center rounded-full bg-surface-2"><X size={20} /></button></div><nav className="flex flex-col gap-2">{items.filter(([, , module]) => !module || modules[module]).map(([label, href]) => { const selected = active === href || (href === "/ayarlar" && active.startsWith("/ayarlar/")); return <Link key={href} href={href} onClick={() => setOpen(false)} className={`flex min-h-12 items-center rounded-field px-4 font-semibold ${selected ? "bg-ink text-ink-contrast" : "hover:bg-surface-2"}`}>{label}</Link>; })}</nav><Link href="/" className="mt-auto rounded-field bg-surface-2 p-4 font-semibold">Siteyi gör ↗</Link></aside></div>}
  </>;
}
