  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] grid place-items-center bg-black/35 px-4 py-8 backdrop-blur-[2px]" onMouseDown={() => !pending && onOpenChange(false)}>
      ...
      >
        ...
        {error && <p role="alert" className="mt-4 rounded-2xl bg-[#fff1f0] p-3 text-sm font-medium text-[#b42318]">{error}</p>}
        <div className="mt-7 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          ...
          <Button type="button" disabled={pending} onClick={confirm} className={variant === "destructive" ? "bg-[#b42318] text-white hover:bg-[#912018]" : undefined}>{pending ? "İşleniyor…" : confirmLabel}</Button>
        </div>
      </div>
    </div>,
    document.body,
