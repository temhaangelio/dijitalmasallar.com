...
export function ActionMenu({ label = "İşlemler", items, trigger, triggerClassName, placement = "anchor" }: { label?: string; items: ActionMenuItem[]; trigger?: ReactNode; triggerClassName?: string; placement?: "anchor" | "center" }) {
  ...
  return (
    <>
      <button ref={triggerRef} type="button" aria-label={label} aria-haspopup="menu" aria-expanded={open} onClick={toggle} className={cn("grid size-9 place-items-center rounded-full text-[#777] transition-colors hover:bg-white hover:text-black", triggerClassName)}>
