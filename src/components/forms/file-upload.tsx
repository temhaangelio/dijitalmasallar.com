...
export function FileUpload({ onChange, label = "Kapak görseli seç" }: { onChange: (file: File | null) => void; label?: string }) {
  ...
  return <div><input ref={inputRef} type="file" accept={accepted.join(",")} className="sr-only" onChange={(event) => select(event.target.files?.[0])} /><button type="button" onClick={() => inputRef.current?.click()} className="flex min-h-28 w-full items-center justify-center gap-3 rounded-field border border-dashed border-line-strong bg-surface-2 px-4 text-sm font-semibold hover:border-ink"><ImagePlus size={20} />{name ?? label}</button>{name && <button type="button" className="mt-2 flex items-center gap-1 text-xs text-muted" onClick={() => { setName(null); onChange(null); }}><X size={13} /> Seçimi kaldır</button>}{error && <p role="alert" className="mt-2 text-[13px] text-danger">{error}</p>}</div>;
}
