...
function renderInline(value: string, keyPrefix: string): ReactNode[] {
  ...
  while ((match = pattern.exec(value)) !== null) {
    ...
    else if (match[8]) nodes.push(<code key={key} className="rounded bg-line px-1.5 py-0.5 font-mono text-[.9em]">{match[8]}</code>);
    else if (match[9]) nodes.push(<del key={key}>{match[9]}</del>);
    else if (match[10]) nodes.push(<mark key={key} className="rounded-[3px] bg-highlight px-0.5 text-inherit">{match[10]}</mark>);
