...
export function PostsStatusTabs({
  ...
  return (
    <div role="tablist" aria-label="Yazı durumu" className="mb-5 grid grid-cols-2 gap-5 sm:grid-cols-3">
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={`flex h-[132px] flex-col justify-between rounded-card p-6 text-left transition-colors ${selected ? "bg-ink text-white" : "bg-surface text-ink hover:bg-surface-2"}`}
          >
            <span className={`text-[15px] font-semibold ${selected ? "text-white" : "text-ink-2"}`}>{tab.label}</span>
            <span className="text-[44px] font-bold leading-none tracking-[-.05em] tabular-nums">{tab.count.toLocaleString("tr-TR")}</span>
          </button>
        );
      })}
    </div>
