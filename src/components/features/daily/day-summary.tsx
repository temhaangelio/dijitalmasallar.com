  ...
  return (
    <section className="mt-1" aria-label="Türkçe ve İngilizce ses kayıtları">
      ...
      {totalTakes ? <div className="mt-4 border-t border-line pt-2">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {speechLanguages.map(language => <div key={language} className="min-w-0"><h4 className="mb-2 text-xs font-semibold">{languageName(language)}</h4><ul className="space-y-2">{takes[language].map((take, index) => <li key={take.id} className="rounded-xl border border-line">
            <div className="flex items-center justify-between gap-2 border-b border-line px-3 py-1"><span className="text-xs text-muted">{take.format.toUpperCase()}</span><a href={`/gunun-ozeti/ses/${take.id}?download=1`} download={`gunun-ozeti-${take.day}-${take.language}.${take.format}`} aria-label={`${languageName(language)} ses kaydını indir`} className={buttonVariants({ variant: "ghost", size: "sm", className: "px-3" })}><Download className="size-4" aria-hidden="true" />İndir</a></div>
            <details className="group/take" open={index === 0}>
              ...
              <div className="space-y-2 px-3 pb-3"><audio controls preload="none" src={`/gunun-ozeti/ses/${take.id}`} className="h-10 w-full" aria-label={`${languageName(language)} ses kaydı`} />
                <div className="flex items-center justify-end gap-1"><Button type="button" variant="ghost" size="sm" className="w-11 px-0" disabled={busy} aria-label={`${languageName(language)} kaydı sil`} onClick={() => void deleteTake(take)}><Trash2 className="size-4" aria-hidden="true" /></Button></div>
