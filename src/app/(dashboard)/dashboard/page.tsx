    ...
    <AppShell active="/dashboard">
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-12">
        ...
        <div className="grid gap-5 sm:grid-cols-2 xl:col-span-2 xl:grid-cols-1">
          <Card className="flex min-h-[140px] flex-col justify-between"><strong>Abone <span className="text-muted">{newsletterData.stats.pending} bekliyor</span></strong><div><div className="text-[40px] font-bold leading-none tracking-[-.05em]">{newsletterData.stats.active.toLocaleString("tr-TR")}</div><small className="text-muted">aktif abone</small></div></Card>
          <Card className="flex min-h-[140px] flex-col justify-between">{nextNewsletter ? <><small className="text-muted">{new Intl.DateTimeFormat("tr-TR", { timeZone, day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }).format(new Date(nextNewsletter.scheduled_at!))}</small><div className="text-[22px] font-bold leading-tight tracking-[-.04em]">#{nextNewsletter.issue_number} <span className="text-muted">{nextNewsletter.subject}</span></div></> : <><small className="text-muted">E-bülten</small><div className="text-[22px] font-bold leading-tight tracking-[-.04em]">Planlı <span className="text-muted">gönderim yok</span></div></>}</Card>
        </div>
