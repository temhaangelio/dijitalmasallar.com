  ...
  return (
    <AppShell active="/dashboard">
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:h-[calc(100dvh-130px)] xl:grid-cols-12 xl:grid-rows-2">
        ...
        <Card className="xl:col-span-6 xl:min-h-0 xl:overflow-hidden xl:p-5"><div className="flex justify-between"><h2 className="section-title">Yayın takvimi</h2><span className="text-muted">{postStats.scheduled.length} planlı yazı</span></div><div className="mt-6 grid grid-cols-7 gap-y-4 text-center xl:mt-4 xl:gap-y-2">{["Pt", "Sa", "Ça", "Pe", "Cu", "Ct", "Pz"].map((label) => <span key={label} className="text-sm font-semibold text-muted">{label}</span>)}{Array.from({ length: firstWeekday - 1 }, (_, index) => <span key={`empty-${index}`} />)}{Array.from({ length: daysInMonth }, (_, index) => { const day = index + 1; return <span key={day} className={scheduledDays.has(day) ? "mx-auto grid size-8 place-items-center rounded-full bg-ink font-semibold text-white xl:size-7" : day === today ? "mx-auto grid size-8 place-items-center rounded-full bg-line font-semibold xl:size-7" : "font-semibold"}>{day}</span>; })}</div></Card>
