...
`posts`, `site_settings`, `ad_units` and `admin_users` belong to the
...
|---|---|
| `posts`, `ad_units`, `site_settings`, `admin_users` | `rls_enabled = true` |
...
| `admin_users` | no `anon` or `authenticated` grants at all — the app only reaches it with `service_role` |
